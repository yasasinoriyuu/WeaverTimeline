...
2026-09-19 Timing 接入补充：带 TimingRows 的块同样保留独立端点按钮，宽块给展开三角额外预留 20 单位，窄块沿用上方入口；替代上文“带 TimingRows 不添加按钮”的旧限制。`GetLaneDisplayHeight(LaneId)` 供消费者原生左栏读取同一展开高度，不复制行高算法。永久 Shutdown 后的控件不得再次挂载，消费者可用 `IsShutdown()` 判断并重建会话。节奏手柄提供左右拖动光标，展开三角朝向与状态一致。

同期核对 `F:\codex\docs\WT与CAK_遗漏核对.md` 与两份 AI 总览：它们基于 WT `709913f` 快照，不代替当前源码核验。TimingRow 命中的终点下界改用钳制后的起点，与绘制及拖动初始化一致；消费者仍须校验输入比例。本轮不扩展多选、框选和组拖动，也不改变冻结标签。上述增量已通过 CineWeaver 的 UE 编译、展开对齐、宽窄块构图及节奏拖动验证，详见消费者 `Docs/TIMING_ROUND.md`；Reference 全套未重跑，不能当作母版全功能重新验收。

TimingRow 松开时补算实际鼠标位置，避免同帧移动事件被合并后提交旧预览值；原位点击不改变值，已取消的手势不复活。该修正来自 CineWeaver 原生窗口鼠标检查发现的“有按下/松开、无中间预览、最后 NoChange”，须同时验证缺少移动事件的指针序列与真实鼠标拖动。

2026-09-19 用户确认 CineWeaver 本轮功能实现并授权推送；当前节奏交互形态仍待用户后续推敲。这是消费端本轮功能确认，不扩展为母版所有能力重新验收。缺失中间移动事件用例和同一末端手柄的真实鼠标复测已通过；消费者最终 28 项检查及另一进程的保存重开检查无失败，四项外部/引擎警告详见消费者记录。
